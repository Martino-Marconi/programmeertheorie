...
def run(routes, max_time, semi_random, semi_random_whc, hill_climber):
    ...
    while ro.train_counter <= routes:
        ...
        # keep adding stations to list as long as train doesn't reach its max time
        while tr.time_travelled <= max_time:
            ...
            # stop if there are no available stations
            if tr.next_station is None:
                break

            # stop if all connections have been used before
            if tr.no_useable_connections():
                break

            # stop if all stations have been visited
            ...
            # add station to train's visited stops
